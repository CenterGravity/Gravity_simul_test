export function calculateGravityForce(m1, m2, pos1, pos2) { // 중력 계산 함수
...
export function updatePositions(bodies, dt) { // 위치 업데이트 함수
  const forces = [];
  for (let i = 0; i < bodies.length; i++) {
    let totalForce = { x: 0, y: 0, z: 0 };
  
    for (let j = 0; j < bodies.length; j++) { // 중력 계산 (F = G * (m1*m2) / r^2)
      if (i === j) continue;
  ...
  for (let i = 0; i < bodies.length; i++) { // 위치와 속도 업데이트
    const a = {                        // 가속도 계산 3차원 물체이기에 3개의 성분을 객체로 묶어서 처리
      x: forces[i].x / bodies[i].mass, // 가속도는 힘 / 질량
      y: forces[i].y / bodies[i].mass, // 가속도는 힘 / 질량
      z: forces[i].z / bodies[i].mass, // 가속도는 힘 / 질량
    };
  
    bodies[i].vel.x += a.x * dt; // 속도 업데이트
    bodies[i].vel.y += a.y * dt;
    bodies[i].vel.z += a.z * dt;
  
    bodies[i].pos.x += bodies[i].vel.x * dt; // 위치 업데이트
